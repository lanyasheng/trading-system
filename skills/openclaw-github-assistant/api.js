...
// Cached user info
let cachedUser = null;
function loadGitHubCredsFromFile() {
    const candidates = [
        path.join(process.env.OPENCLAW_HOME || path.join(process.env.HOME || '', '.openclaw'), 'credentials', 'github.json'),
        path.join(process.env.HOME || '', '.openclaw', 'credentials', 'github.json')
    ];
...
function getAuthHeaders(context) {
    ...
    // 3. Fallback to local credentials file
    const fileCreds = loadGitHubCredsFromFile();
    if (fileCreds.token) {
        headers['Authorization'] = `token ${fileCreds.token}`;
    }
    return headers;
}
/**
...
async function getUsername(context) {
    ...
    const fileCreds = loadGitHubCredsFromFile();
    if (fileCreds.username) {
        return fileCreds.username;
    }
    // 4. Fetch from API if not configured
    if (!cachedUser) {
